...
import { Clock, AlertCircle, CheckCircle, Pencil, Trash2, History, Mail, FilePlus2, ArrowRight } from 'lucide-react';
import { useCambiosPendientes, useAceptarCambio, useCancelarCambio, useBorradoresEstado } from '../../hooks/useCambiosTurno';
...
export default function ListaPendientes() {
  ...
  const { data: pendientes = [], isLoading } = useCambiosPendientes();
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-auto px-2 py-0.5 rounded-full bg-outline-variant/20 text-xs font-medium">
          {pendientes.length}
        </span>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {pendientes.map((p: CambioListado) => {
            const modo = getModo(p);
              ...
              <div key={p.id_transaccion} className="p-4 rounded-lg border border-outline-variant/20 bg-surface">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${estadoColor[p.estado || 'PENDIENTE'] || 'bg-gray-100 text-gray-800 border-gray-300'}`}>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {/* Zona correo: chips de estado + switch + crear + editar */}
                  <div className="flex flex-wrap items-center gap-2 ml-auto">
                    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${pendienteCreado ? 'bg-green-50 text-green-700 border-green-300' : 'bg-amber-50 text-amber-700 border-amber-300'}`}>
                      {pendienteCreado ? '✔' : '✖'} Pendiente
                    </span>
                    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${confirmadoCreado ? 'bg-green-50 text-green-700 border-green-300' : 'bg-amber-50 text-amber-700 border-amber-300'}`}>
